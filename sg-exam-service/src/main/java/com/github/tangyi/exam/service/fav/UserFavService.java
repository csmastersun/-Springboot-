...
public class UserFavService extends CrudService<UserFavMapper, ExamUserFav>
  ...
  public PageInfo<ExamUserFav> findUserFavoritesPage(
      Map<String, Object> params, int pageNum, int pageSize, String targetType) {
  ...
  public String getFavCountKey(int targetType) {
    ...
    switch (targetType) {
      case FAV_TYPE_EXAM:
        key = USER_FAVORITES_EXAMINATION_COUNT;
        break;
      case FAV_TYPE_COURSE:
        key = USER_FAVORITES_COURSE_COUNT;
        break;
      case FAV_TYPE_SUBJECT:
        key = USER_FAVORITES_SUBJECT_COUNT;
        break;
      default:
    }
  ...
  public String getUserFavKey(int targetType) {
    ...
    switch (targetType) {
      case FAV_TYPE_EXAM:
        key = USER_FAVORITES_EXAMINATION;
        break;
      case FAV_TYPE_COURSE:
        key = USER_FAVORITES_COURSE;
        break;
      case FAV_TYPE_SUBJECT:
        key = USER_FAVORITES_SUBJECT;
        break;
      default:
        break;
    }
  ...
  public String getStartCountKey(int targetType) {
    ...
    switch (targetType) {
      case FAV_TYPE_EXAM:
        key = EXAMINATION_START_COUNT;
        break;
      case FAV_TYPE_COURSE:
        key = COURSE_START_COUNT;
        break;
      case FAV_TYPE_SUBJECT:
        key = SUBJECT_START_COUNT;
        break;
      default:
        break;
    }
      ...
      for (Object fieldObj : fields) {
        ...
        if (value != null) {
          ...
          Stream.of(value.toString().split(","))
              .filter(StringUtils::isNotBlank)
              .forEach(
                  eId -> user2FavIdMap.computeIfAbsent(userId, s -> Sets.newHashSet()).add(eId));
        }
      }
    }
  ...
  public void updateFavRelation(int targetType) {
    ...
    if (MapUtils.isNotEmpty(user2FavIdMap)) {
      ...
      for (ExamUserFav fav : favorites) {
        ...
        if (tempFav == null) {
          fav.setCommonValue(
              TenantConstant.DEFAULT_TENANT_CODE, TenantConstant.DEFAULT_TENANT_CODE);
          insert(fav);
        }
      }
    }
