...
public class ExaminationService extends CrudService<ExaminationMapper, Examination>
  ...
  public ExaminationDto toDto(Examination exam, List<Course> courses) {
    ...
    // 设置考试所属课程
    courses.stream()
        .filter(tempCourse -> tempCourse.getId().equals(exam.getCourseId()))
        .findFirst()
  ...
  public void initExaminationImage(ExaminationDto examinationDto) {
    try {
      if (examinationDto.getImageId() != null && examinationDto.getImageId() != 0L) {
        ...
        examinationDto.setImageUrl(
            StringUtils.getIfEmpty(imageUrl, randomImageService::randomImage));
      }
    } catch (Exception e) {
  ...
  @Transactional
  @CacheEvict(
      value = {ExamCacheName.EXAMINATION, ExamCacheName.EXAM_ALL_SUBJECT},
      key = "#examinationDto.id")
  ...
  public void deleteExaminationSubject(Long[] ids) {
    for (Long id : ids) {
      List<ExaminationSubject> examinationSubjects =
          examinationSubjectService.findListByExaminationId(id);
      if (CollectionUtils.isNotEmpty(examinationSubjects)) {
        ExaminationSubject es = new ExaminationSubject();
        examinationSubjects.forEach(
            examinationSubject -> {
  ...
  public PageInfo<SubjectDto> findSubjectPageById(
      SubjectDto subjectDto, Map<String, Object> params, int pageNum, int pageSize) {
    // 返回结果
    ...
    PageInfo<ExaminationSubject> examinationSubjects =
        examinationSubjectService.findPage(params, pageNum, pageSize);
    // 根据题目ID查询题目信息
    List<SubjectDto> subjectDtoList = Lists.newArrayList();
    if (CollectionUtils.isNotEmpty(examinationSubjects.getList())) {
      Long[] subjectIds =
          examinationSubjects.getList().stream()
              .map(ExaminationSubject::getSubjectId)
              .toArray(Long[]::new);
    ...
    if (CollectionUtils.isNotEmpty(subjectDtoList)) {
      subjectDtoList =
          subjectDtoList.stream()
              .sorted(Comparator.comparing(SubjectDto::getSort))
  ...
  public List<SubjectDto> allSubjectList(SubjectDto subjectDto) {
    // 查询考试题目关联表
    List<ExaminationSubject> esList =
        examinationSubjectService.findListByExaminationId(subjectDto.getExaminationId());
    if (CollectionUtils.isNotEmpty(esList)) {
      Long[] subjectIds =
          esList.stream().map(ExaminationSubject::getSubjectId).toArray(Long[]::new);
      List<Subjects> subjects = subjectsService.findBySubjectIds(subjectIds);
  ...
  public byte[] produceCode(Long examinationId) {
    ...
    // 调查问卷
    if (examination
        == null /* || !ExaminationTypeEnum.QUESTIONNAIRE.getValue().equals(examination.getType())*/) {
      return new byte[0];
    }
  ...
  public List<SimpleSubjectDto> allSubjects(Long examinationId) {
    ...
    if (CollectionUtils.isNotEmpty(ess)) {
      ...
      List<SubjectDto> dtoList =
          subjectsService.findSubjectDtoList(
              subjectsService.findBySubjectIds(subjectIds), true, false);
  ...
  public List<SubjectDto> randomAddSubject(List<Subjects> subjects, int cnt) {
    ...
    while (tmpSubjects.size() < cnt) {
      ...
      if (!idSet.contains(tmp.getId())) {
        ...
        tmpSubjects.add(tmp);
        log.info(
            "select subject: {}，size: {}, target size: {}", tmp.getId(), tmpSubjects.size(), cnt);
      }
    }
