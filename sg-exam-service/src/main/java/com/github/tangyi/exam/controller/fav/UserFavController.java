...
public class UserFavController extends BaseController {
  ...
  public R<PageInfo<ExamUserFav>> list(
  ...
  public R<Boolean> update(
      @PathVariable("id") Long id, @RequestBody @Valid ExamUserFav examUserFav) {
  ...
  public R<Boolean> favExam(
      @PathVariable Long examinationId, @RequestParam Long userId, @RequestParam Integer type) {
  ...
  public R<Boolean> favSubject(
      @PathVariable Long subjectId, @RequestParam Long userId, @RequestParam Integer type) {
  ...
  public R<Boolean> favCourse(
      @PathVariable Long courseId, @RequestParam Long userId, @RequestParam Integer type) {
    return R.success(courseFavoritesService.favorite(userId, courseId, type));
  }

  @GetMapping("user/favorites")
  @Operation(summary = "查询用户收藏列表，支持考试/课程/题目类型")
  public R<PageInfo<?>> favorites(
      @RequestParam Map<String, Object> condition,
      @RequestParam(value = PAGE, required = false, defaultValue = PAGE_DEFAULT) int pageNum,
      @RequestParam(value = PAGE_SIZE, required = false, defaultValue = PAGE_SIZE_DEFAULT)
          int pageSize) {
    String targetType =
        condition.getOrDefault("targetType", UserFavConstant.FAV_TYPE_EXAM).toString();
    PageInfo<ExamUserFav> page =
        userFavService.findUserFavoritesPage(condition, pageNum, pageSize, targetType);
    if (CollectionUtils.isNotEmpty(page.getList())) {
      int targetTypeInt = Integer.parseInt(targetType);
      PageInfo<?> result;
      if (targetTypeInt == UserFavConstant.FAV_TYPE_EXAM) {
        result = examinationService.findUserFavoritesPage(page);
      } else if (targetTypeInt == UserFavConstant.FAV_TYPE_COURSE) {
        result = courseService.findUserFavoritesPage(page);
      } else if (targetTypeInt == UserFavConstant.FAV_TYPE_SUBJECT) {
        result = subjectsService.findUserFavoritesPage(page);
      } else {
        result = new PageInfo<>();
      }
