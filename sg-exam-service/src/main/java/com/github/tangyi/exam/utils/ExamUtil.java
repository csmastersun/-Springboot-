...
public class ExamUtil {
  ...
  public static Map<Integer, List<Answer>> distinctAnswer(
      List<Answer> answers, Map<Long, Integer> typeMap) {
    Map<Integer, List<Answer>> map = Maps.newHashMap();
    map.putAll(
        answers.stream()
            .collect(
                Collectors.groupingBy(s -> typeMap.get(s.getSubjectId()), Collectors.toList())));
    return map;
  }

  public static List<ExamRecordExcelModel> convertExamRecord(
      List<ExaminationRecordDto> examinationRecords) {
    List<ExamRecordExcelModel> examRecordExcelModels = new ArrayList<>(examinationRecords.size());
    examinationRecords.forEach(
        examinationRecord -> {
          ...
          if (CollectionUtils.isNotEmpty(subject.getOptions())) {
            for (SubjectOption option : subject.getOptions()) {
              switch (option.getOptionName()) {
                case "A":
                  subjectExcelModel.setOptionA(option.getOptionContent());
                  break;
                case "B":
                  subjectExcelModel.setOptionB(option.getOptionContent());
                  break;
                case "C":
                  subjectExcelModel.setOptionC(option.getOptionContent());
                  break;
                case "D":
                  subjectExcelModel.setOptionD(option.getOptionContent());
                  break;
                default:
              }
            }
