...
public class QiNiuService implements IQiNiuService {
  ...
  public static final String DEFAULT_COURSE_IMAGE = "DefaultCourseImage";
  public static final String DEFAULT_EXAM_IMAGE = "DefaultExamImage";
  public static final String DEFAULT_USER_IMAGE = "DefaultUserImage";
  ...
  @Transactional
  public Attachment upload(MultipartFile file, String groupCode, String user, String tenantCode)
      throws IOException {
    return upload(
        groupCode,
  ...
  public void upload(Attachment attachment, byte[] bytes) {
    ...
    try {
      ...
      DefaultPutRet putRet =
          JsonMapper.getInstance().fromJson(response.bodyString(), DefaultPutRet.class);
  ...
  @Transactional
  @CacheEvict(
      value = {UserCacheName.ATTACHMENT, UserCacheName.ATTACHMENT_URL},
      key = "#attachment.id")
  ...
  public boolean delete(String fileName) throws QiniuException {
    log.info("Deleting file {} ...", fileName);
    getBucketManager(getAuth(), new Configuration(Region.region2()))
        .delete(qiNiuConfig.getBucket(), fileName);
    log.info("File has been deleted, fileName: {}", fileName);
    return Boolean.TRUE;
  }
  ...
  public Long randomImage(String groupCode) {
    String name =
        RandomImageUtil.randomImage(props.getDefaultImageCount(), props.getDefaultImageType());
    ...
    attachmentService.insert(attachment);
    log.info(
        "Generate random image successfully, groupCode: {}, url: {}, id: {}",
        groupCode,
        attachment.getUrl(),
        attachment.getId());
  ...
  @Override
  public Long getDefaultCourseImage() {
    return Optional.ofNullable(attachmentService.findByName(DEFAULT_COURSE_IMAGE))
        .map(Attachment::getId)
        .orElse(0L);
  }

  @Override
  public Long getDefaultExamImage() {
    return attachmentService
        .findList(Attachment.builder().attachName(DEFAULT_EXAM_IMAGE).build())
        .stream()
        .findFirst()
        .map(Attachment::getId)
        .orElse(0L);
  ...
  @Override
  public Long getDefaultUserImage() {
    return attachmentService
        .findList(Attachment.builder().attachName(DEFAULT_USER_IMAGE).build())
        .stream()
        .findFirst()
        .map(Attachment::getId)
        .orElse(0L);
  ...
  private Auth getAuth() {
    if (StringUtils.isNotBlank(qiNiuConfig.getAccessKey())
        && StringUtils.isNotBlank(qiNiuConfig.getSecretKey())) {
